...
//
// La función valida el JWT del llamante y comprueba en la tabla
// public.profiles que su rol sea 'super_admin'. Si no lo es, rechaza.
//
// Deploy:
...
serve(async (req) => {
  ...
  // Cliente con el token del llamante — para validar quién es
  ...
  try {
    ...
    if (action === "setEmail") {
      ...
      const { error } = await admin.auth.admin.updateUserById(targetId, {
        email,
        email_confirm: true, // no requerir confirmación por link
      });
      if (error) return json({ error: error.message }, 400);
